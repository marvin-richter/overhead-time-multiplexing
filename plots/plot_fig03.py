...
from plot_settings import DOUBLE_COLUMN_WIDTH, SINGLE_COLUMN_WIDTH, wong_colors, ROOT
...
def add_grid_markers(ax):
    """Add vertical reference lines for 5×5 and 11×11 grid sizes."""
    for x, label in [(25, r" 5$\times$5"), (121, r" 11$\times$11")]:
        ax.axvline(x=x, color="gray")
        ax.text(
            x, 0.07, label, va="top", ha="left", transform=ax.get_xaxis_transform(), fontsize=8
        )
...
def plot_figure() -> plt.Figure:
    ...
    df = pl.concat(
        [
            pl.read_parquet(
                ROOT / "results" / "20260122-133503_grids_random_densities_N880.parquet"
            ),
            pl.read_parquet(
                ROOT / "results" / "20260122-152622_grids_random_densities_N60.parquet"
            ),
        ]
    ).filter(pl.col("hardware.num_qubits") > 9)
    ...
    # Fit T_trans for use in panel (b)
    x = df_k2["hardware.num_qubits"].to_numpy()
    y = df_k2["T_trans_median"].to_numpy()
    y_lo = df_k2["T_trans_q25"].to_numpy()
    y_hi = df_k2["T_trans_q75"].to_numpy()
    n = df_k2["count"].to_numpy()
    a_trans, b_trans, *_ = fit_power_law(x, y, y_lo, y_hi, x_min=fit_x_min, n_samples=n)
    print(f"Target circuits: a={a_trans:.2e}, b={b_trans:.3f}")

    # Target circuit duration reference band
    median_trans = df["T_trans"].median()
    q25_trans = df["T_trans"].quantile(0.25)
    q75_trans = df["T_trans"].quantile(0.75)
    ax.axhline(y=median_trans, color="black", linewidth=1, alpha=0.7)
    ax.axhspan(q25_trans, q75_trans, color="gray", alpha=0.2, zorder=0)
    ax.text(
        0.98,
        median_trans * 1.1,
        "Target circuits",
